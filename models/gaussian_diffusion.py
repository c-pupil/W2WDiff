...
def get_first_stage_encoding(encoder_posterior):
    ...
    raise TypeError(f"Unsupported encoder posterior: {type(encoder_posterior)!r}")
...
class GaussianDiffusion:
    ...
    def p_sample(self, model, x, y, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, noise_repeat=False,guidance=None):
        """
    ...
    def p_sample_loop(
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            final = final.to(memory_format=torch.contiguous_format).float() / 0.18215
        with th.no_grad():
            if self.enc_fea is not None:
                out = self.decode_first_stage(
                    final,
                    first_stage_model=first_stage_model,
                    consistencydecoder=consistencydecoder,
                    enc_fea=self.enc_fea,
                )
    ...
    def p_sample_loop_progressive(
        ...
        with torch.no_grad():
            result = self.encode_first_stage(y, first_stage_model, up_sample=True)
        ...
        print(f"Sampling with {len(indices)} diffusion steps")
    ...
    def encode_first_stage(self, z_sample, first_stage_model=None, up_sample=True):

        sf = 1

        data_dtype = z_sample.dtype
        model_dtype = next(first_stage_model.parameters()).dtype
        if up_sample and sf != 1:
            z_sample = F.interpolate(z_sample, scale_factor=sf, mode='bicubic')
        if first_stage_model is None:
            ...
            with torch.no_grad():
                z_y,enc_fea = first_stage_model.encode(z_sample)



                z_y = get_first_stage_encoding(z_y).detach()
    ...
    def decode_first_stage(self, z_sample, first_stage_model=None, consistencydecoder=None, predict_cids=False,
                           force_not_quantize=False,enc_fea=None):
        ...
        if first_stage_model is None:
            ...
            if consistencydecoder is None:
                out = decoder(z_sample.type(model_dtype), enc_fea)
            else:
                with torch.cuda.amp.autocast():
                    out = decoder(z_sample,enc_fea)
            if not model_dtype == data_dtype:
                out = out.type(data_dtype)
            return out
